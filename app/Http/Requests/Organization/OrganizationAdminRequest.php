...
class OrganizationAdminRequest extends FormRequest
{
    ...
    public function rules()
    {
        switch ($this->method()) {
            case 'POST':
            {
                ...
                $rules['users.organization_id'] = 'required';
                $rules['user_programs.program_id'] = 'required';
                $rules['users.email'] = 'required|email|max:199unique:users,email,'.$this->admin;
                $rules['users.phone'] = 'required|min:10|max:50';
                // $rules['userDetail.dob'] = 'required|max:12';
                $rules['users.address'] = 'required';
                $rules['users.city'] = 'required|max:50';
                $rules['users.state_id'] = 'required';
                $rules['users.zip'] = 'required';
                // $rules['userDetail.gender'] = 'required|max:2';
            ...
            case 'PUT':
            {
                ...
                $rules['users.organization_id'] = 'required';
                $rules['user_programs.program_id'] = 'required';
                $rules['users.email'] = 'required|email|max:199|unique:users,email,'.$this->admin;
                $rules['users.password'] = 'nullable|required_with:password_confirmation|same:password_confirmation|min:8|regex:/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/';
                $rules['users.phone'] = 'required|min:10|max:50';
                $rules['users.address'] = 'required';
                $rules['users.city'] = 'required|max:50';
                $rules['users.state_id'] = 'required';
    ...
    public function messages()
    {
        ...
        $ruleMsg['users.organization_id.required'] = 'Please select the organization';
        $ruleMsg['user_programs.program_id.required'] = 'Please select the programs.';
        ...
        $ruleMsg['users.password.min'] = 'The password must be at least 8 or more characters, at least one uppercase, at least one lowercase and a special character.';
        $ruleMsg['users.password.regex'] = 'The password must be at least 8 or more characters, at least one uppercase, at least one lowercase and a special character.';
        ...
        $ruleMsg['users.state_id.required'] = 'Please select the state.';
    ...
    public function withValidator($validator)
    {
        if (! $validator->fails()) {
            $input = $this->all();
            if($this->method() == 'POST'){
                $input['users']['created_by'] = Auth::id();
                $input['users']['is_active'] = '1';
